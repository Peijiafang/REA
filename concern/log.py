...
class Logger(Configurable):
    ...
    def report_eta(self, steps, total, epoch, total_epoch):
        self.logged = self.logged % total + 1
        steps_epoch = steps % total
        ...
        seconds_epoch = (total - steps_epoch) * speed
        hours_epoch = seconds_epoch // 3600
        minutes_epoch = (seconds_epoch - (hours_epoch * 3600)) // 60
        seconds_epoch = seconds_epoch % 60

        seconds_total = (total * (total_epoch + 1) - steps) * speed
        hours_total = seconds_total // 3600
        minutes_total = (seconds_total - (hours_total * 3600)) // 60
        seconds_total = seconds_total % 60

        print('%d/%d batches processed in epoch %d, ETA_epoch: %2d:%2d:%2d, ETA_total: %3d:%2d:%2d' %
              (steps_epoch, total, epoch,
               hours_epoch, minutes_epoch, seconds_epoch,
               hours_total, minutes_total, seconds_total), end='\r')
