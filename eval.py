    ...
    def eval(self, visualize=False):
        ...
        with torch.no_grad():
            for _, data_loader in self.data_loaders.items():
                raw_metrics = []
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader), ncols=80):
                    ...
                    if visualize and self.structure.visualizer:
                        ...
                        # vis_images.update(vis_image)  # memory increase error
                metrics = self.structure.measurer.gather_measure(raw_metrics, self.logger)
                for key, metric in metrics.items():
                    self.logger.info('%s : %.4f (%d)' % (key, metric.avg, metric.count))

                t = pt.PrettyTable()
                t.set_style(pt.MSWORD_FRIENDLY)
                t.add_column('th', [self.args['box_thresh']])
                ...
                for key, metric in metrics.items():
                    if key == 'precision':
                        ...
                        fieldname = 'IOU_care'
                    t.add_column(fieldname, ["{:.4f}".format(metric.avg)])
                print(t)
