...
class AdaptiveInstanceDiceLoss(nn.Module):
    ...
    def forward(self, pred, batch):
        ...
        loss = self.patial_loss(self.weights['main'], main_loss) \
...
class L1BalanceCELoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5, zooming_scale=1.5, seg_scale=1):
        super(L1BalanceCELoss, self).__init__()
        from .dice_loss import DiceLoss
        from .l1_loss import MaskL1Loss
        from .balance_cross_entropy_loss import BalanceCrossEntropyLoss
        self.dice_loss = DiceLoss(eps=eps)
        self.select_dice_loss = DiceLoss(eps=eps)
        self.l1_loss = MaskL1Loss()
        ...
        self.zooming_scale = zooming_scale
    ...
    def forward(self, pred, batch):
        ...
        if 'thresh' in pred:
            ...
            loss = bce_loss
        loss = self.seg_scale * loss

        if 'zooming_preds' in pred:
            bce_loss = self.bce_loss(pred['zooming_preds']['binary'], batch['gt'], batch['mask'])
            metrics.update(zooming_bce_loss=bce_loss)
            if 'thresh' in pred:
                l1_loss, l1_metric = self.l1_loss(pred['zooming_preds']['thresh'], batch['thresh_map'], batch['thresh_mask'])
                dice_loss = self.dice_loss(pred['zooming_preds']['thresh_binary'], batch['gt'], batch['mask'])
                loss = loss + self.zooming_scale * (dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale)
                metrics.update(zooming_l1_loss=l1_loss)
                metrics.update(zooming_thresh_loss=dice_loss)
            else:
                loss = loss + self.zooming_scale * bce_loss
            select_loss = self.bce_loss(pred['zooming_preds']['select'], batch['gt'], batch['mask'])
            loss = loss + select_loss * self.bce_scale
            metrics.update(select_loss=select_loss)
        return loss, metrics
...
class SRL1BalanceCELoss(nn.Module):

    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5, sr_scale=10):
        super(SRL1BalanceCELoss, self).__init__()
        from .dice_loss import DiceLoss
        from .l1_loss import MaskL1Loss, L1Loss
        from .balance_cross_entropy_loss import BalanceCrossEntropyLoss
        self.dice_loss = DiceLoss(eps=eps)
        self.l1_loss = MaskL1Loss()
        self.bce_loss = BalanceCrossEntropyLoss()
    ...
    def forward(self, pred, batch):
        ...
        if 'thresh' in pred:
            l1_loss, l1_metric = self.l1_loss(pred['thresh'], batch['thresh_map'], batch['thresh_mask'])
            dice_loss = self.dice_loss(pred['thresh_binary'], batch['gt'], batch['mask'])
            sr_loss, sr_metric = self.sr_loss(pred['sr'], batch['image'])
            metrics['thresh_loss'] = dice_loss

            loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale + self.sr_scale * sr_loss
            metrics.update(**l1_metric)
            metrics.update(**sr_metric)
        else:
            loss = bce_loss
        return loss, metrics
