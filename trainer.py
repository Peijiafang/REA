...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            for batch in train_data_loader:
                ...
                self.steps += 1
                self.logger.report_eta(self.steps, self.total, epoch, self.experiment.train.epochs)
    ...
    def train_step(self, model, optimizer, batch, epoch, step, **kwards):
        ...
        if step % self.experiment.logger.log_interval == 0:
            ...
            if 'image_blur' in batch:
                self.logger.add_image('SR/image/image', utils.make_grid(batch['image'][:, (2, 1, 0), :, :], normalize=True, scale_each=True), step)
                self.logger.add_image('SR/image/image_blur', utils.make_grid(batch['image_blur'][:, (2, 1, 0), :, :], normalize=True, scale_each=True), step)
                self.logger.add_image('SR/image/sr', utils.make_grid(pred['sr'][:, (2, 1, 0), :, :], normalize=True, scale_each=True), step)

            if 'zooming_preds' in pred:
                self.logger.add_image('zooming/image', utils.make_grid(pred['zooming_preds']['zooming_image'][:, (2, 1, 0), :, :], normalize=True, scale_each=True), step)
                if 'thresh_binary' in pred['zooming_preds']:
                    self.logger.add_image('zooming/pred/zooming_binary', utils.make_grid(pred['zooming_preds']['thresh_binary']), step)
                else:
                    self.logger.add_image('zooming/pred/zooming_prob', utils.make_grid(pred['binary']), step)
                self.logger.add_image('zooming/pred/select', utils.make_grid(pred['zooming_preds']['select']), step)

            self.logger.add_image('DB/image', utils.make_grid(batch['image'][:, (2, 1, 0), :, :], normalize=True, scale_each=True), step)
            self.logger.add_image('DB/gt', utils.make_grid(batch['gt']), step)
            # self.logger.add_image('DB/image/thresh_map', utils.make_grid(batch['thresh_map'].unsqueeze(1)), step)

            # self.logger.add_image('DB/pred/th', utils.make_grid(pred['thresh']), step)
            if 'thresh_binary' in pred:
                self.logger.add_image('DB/pred/binary', utils.make_grid(pred['thresh_binary']), step)
            else:
                self.logger.add_image('DB/pred/prob', utils.make_grid(pred['binary']), step)

            for name, metric in metrics.items():
                self.logger.add_scalar("loss/" + name, metric.mean(), step)
    ...
    def validate_step(self, data_loader, model, visualize=False):
        ...
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            raw_metric = self.structure.measurer.validate_measure(batch, output, is_output_polygon=False, box_thresh=0.6)
