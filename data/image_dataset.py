...
class ImageDataset(data.Dataset, Configurable):
    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list if 'gif' not in timg]
                gt_path=[self.data_dir[i]+'/train_gts/'+timg.strip()+'.txt' for timg in image_list if 'gif' not in timg]
            else:
    ...
    def load_ann(self):
        ...
        for gt in self.gt_paths:
            ...
            for line in reader:
                ...
                if 'icdar' in self.data_dir[0] or 'ic' in self.data_dir[0]:
