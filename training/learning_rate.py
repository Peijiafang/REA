...
class PiecewiseConstantLearningRate(Configurable):
    boundaries = State(default=[25000, 50000, 75000, 100000])
    values = State(default=[0.007, 0.001, 0.0003, 0.00006, 0.00001])
...
class ExpDecayLearningRate(Configurable):
    lr = State(default=0.007)
    decay_step = State(default=1600)
    min_lr = State(default=0.00001)
    factor = State(default=0.9)
    # factor ^ (iter/decay_iter) * lr

    def __init__(self, **kwargs):
        self.load_all(**kwargs)

    def get_learning_rate(self, epoch, step):
        rate = np.power(self.factor, step / self.decay_step)
        lr = np.max([self.min_lr, rate * self.lr])
        return lr
